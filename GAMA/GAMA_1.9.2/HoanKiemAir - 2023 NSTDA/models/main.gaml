...
global { 
	...
	init { 		
		...
		create param_indicator with: [x::3100, y::803, size::30, name::"Time", value::"00:00:00"];		
	...
	float general_traffic_daytime {
		if(daytime_traffic_peak.keys one_matches (each.hour = current_date.hour 
			and each.minute = current_date.minute and each.second = current_date.second)){
			// Only work when only one peak per hour
			return daytime_traffic_peak[daytime_traffic_peak.keys first_with (each.hour = current_date.hour)];	
		} else {
			date fd <- (daytime_traffic_peak.keys where (each.hour > current_date.hour)) with_min_of (each.hour - current_date.hour);
			if(fd = nil){ fd <- first(daytime_traffic_peak.keys); }
			date pd <- (daytime_traffic_peak.keys where (each.hour <= current_date.hour)) with_min_of (current_date.hour - each.hour);
			if(pd = nil){ pd <- last(daytime_traffic_peak.keys); }
