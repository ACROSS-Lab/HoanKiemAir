...
//************************************************************************************************************************************************************
	...
	shape_file trafic_show <- shape_file("../includes/traffic_show.shp");
	shape_file shape_file_buildings <- shape_file("../includes/buildings.shp");
	shape_file shape_file_river <- shape_file("../includes/lake.shp");
	shape_file shape_file_lakes <- shape_file("../includes/lakes.shp");
	shape_file node_shape <- shape_file("../includes/roads_nodes.shp");

	shape_file cell_shape <- shape_file("../includes/traffic_show.shp"); // file("../includes/SIG_demonstrateur/road_cells.shp");
	shape_file shape_file_bound <- shape_file("../includes/keystone_HKA.shp");

	
	// A REMETTRE : dummy road : roads supplémentaires sans influence sur le traffic
	file PM <- file("../includes/node_generator_trafic.shp");	
	...
	//COMPTAGES ROUTIER
	matrix countingData <- matrix(csv_file( '../includes/csv/PM_Test.csv', ';'));
	
	//MATRICE DE CHOIX ROUTIER
	matrix<float> HierarchyMatrix <- matrix<float>(csv_file( '../includes/csv/HierarchyChoice.csv', ';'));
	matrix<float> SpeedMatrix <- matrix<float>(csv_file( '../includes/csv/SpeedChoice.csv', ';'));
...
//************************************************************************************************************************************************************	
	...
	float maxNox <- 2500.0; //1.0 update: max(pollutant_grid collect(each.pollutant[world.pollutentIndex("nox")]));
...
//	float diffusion_rate <- 0.5;  // modif alived
	...
	map<float,list<list<float>>> readCopertData(string fileName)
	{
		...
		list<list<float>> cols <- rows_list(copert) ;//as list<list<float>>;
		int i <- 0;
	...
	int pollutentIndex(string name_pol)
	{
		int idx <- 0;
		switch(name_pol)
		{
	...
	action initialize
	{
		...
		copert <- copert + ("2007"::readCopertData( '../includes/csv/CopertData2007.csv'));
		copert <- copert + ("2020"::readCopertData( '../includes/csv/CopertData2020.csv'));		
...
//			}
		...
		create landscape from: shape_file_lakes
		{
			mycolor <- first(colorSet).WATER;
		}
		create landscape from: shape_file_river
		{
			mycolor <- first(colorSet).WATER;
		}
...
//			alived_building <- alived_building accumulate(neighboor_buildings);   // modif alived
		...
		create road from: shape_file_roads with:[mid::int(read("osm_id")), oneway::int(read("oneway")),hierarchy::float(read("TYPE_OSM_C")),roundaboutId::int(read("roundabout")),mspeed::float(read("maxspeed"))#km/#h]
		{
			...
			if(mspeed = 0.0) { mspeed <- MAX_SPEED;}
...
species carCounter schedules: ( (time - last_reset_time) mod 1#mn ) = 0 ? carCounter: []
{
	...
	road associatedRoad;
	float nbCar_digit_to_create <- 0.0;
	float nbCar_ndigit_to_create <- 0.0;
	
	//Nombre de véhicules créés pour la période
	float nbCar_created <- 0.0;
...
species car skills: [driving]
{
	...
	reflex cloud
	{
		...
		write sample(my_vehicle_year);
		write sample(spp);
		write sample(my_energy);
		write sample(world.copert at ("" + my_vehicle_year));
		write sample( ((world.copert at ("" + my_vehicle_year) ) at spp));
		
		list<float> mcopert <- ((world.copert at ("" + my_vehicle_year) ) at spp)[my_energy];
