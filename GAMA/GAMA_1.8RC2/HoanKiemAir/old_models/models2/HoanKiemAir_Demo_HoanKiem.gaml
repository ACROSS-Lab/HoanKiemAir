...
global {
	...
	shape_file roads_shape_file <- shape_file("../includes/roads_scenario0_noClose.shp"); // shape_file("../includes/roads.shp");	
	...
	action update_and_create_agents {
		...
		if(delta_nb_moto < 0) {
			...
			write "Create moto " + delta_nb_moto;
		...
		if(delta_nb_car < 0) {
			...
			write "Create auto " + delta_nb_car;
...
//	shape_file roads_scenario0_noClose0_shape_file <- shape_file("../includes/roads_scenario0_noClose.shp");
	...
	init {
		create simulation with: [roads_shape_file::roads_scenario1_normal0_shape_file];
