...
//			is_traffic_signal <- true;
	...
	reflex update_network_weights {
		...
		map<road, float> road_weights <- road as_map (each::(each.shape.perimeter + length(each.all_agents) * 7));
		...
		if (delta < 0) {
			...
			create vehicle number: delta {
				...
				if (type = "car") {
					...
					self.max_speed <- gauss(50.0, 5.0);
					self.color <- #orange;
				} else {
					self.vehicle_length <- 2.0#m;
					self.max_speed <- gauss(40.0, 7.0);
					self.color <- #cyan;
				}
			}
		}
